      ...
      <form onSubmit={handleSubmit} className="max-w-3xl mx-auto">
        <div className="relative flex items-center">
          ...
          <input
            ...
            placeholder="Paste candidate URL (e.g. https://domain.com/login or http://192.168.1.1)..."
            className="w-full pl-12 pr-36 py-4 rounded-xl bg-slate-900/90 border border-slate-700/80 text-white placeholder-slate-500 font-mono text-sm focus:outline-none focus:border-cyan-400 focus:ring-2 focus:ring-cyan-500/20 transition shadow-inner"
            disabled={loading}
          />
          <button
            type="submit"
            disabled={loading || !inputUrl.trim()}
            className="absolute right-2 px-5 py-2.5 rounded-lg bg-gradient-to-r from-cyan-500 to-blue-600 hover:from-cyan-400 hover:to-blue-500 text-white text-xs font-bold font-mono tracking-wider transition-all disabled:opacity-50 flex items-center space-x-1.5 shadow-lg shadow-cyan-500/20"
          >
            {loading ? (
              <>
